...
class EvaluationLaunchRequest(BaseModel):
    ...
    controllers: list[str] = Field(
        default_factory=lambda: ["sequential", "greedy", "auction", "cp_sat"]
    )
    ...
    *,
    ...
    def launch_evaluation(payload: EvaluationLaunchRequest) -> dict[str, str]:
        ...
        for controller in ("mappo", "ippo"):
            ...
            checkpoints[controller] = record["manifest"]["checkpoint_path"]
    ...
    async def stream_run_events(websocket: WebSocket, run_id: str) -> None:
        ...
        while True:
            ...
            if current and current["status"] in TERMINAL_RUN_STATUSES and not events:
