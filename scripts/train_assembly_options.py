...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the hierarchical team-options assembly policy.")
    ...
    parser.add_argument("--metrics", default=str(PROJECT_ROOT / "logs" / "assembly_option_training_metrics.json"))
...
def main() -> int:
    ...
    trainer = HierarchicalOptionTrainer(env=env, config=train_config, device=args.device or runtime_profile.device)
    summary = trainer.train(checkpoint_path=Path(args.checkpoint), metrics_path=Path(args.metrics))

    print("Hierarchical options training complete")
    ...
    print(f"Last mean beams installed: {summary.last_mean_beams_installed:.3f}")
    print(f"Scripted baseline success rate: {summary.scripted_success_rate:.3f}")
